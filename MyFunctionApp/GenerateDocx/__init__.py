...
def generate_docx_from_knowledge_scan(scan_data):
    ...
    overall_summary = scan_data.get('overall_summary', 'No overall summary available.')  # Correctly fetch the overall summary generated by GPT
    references = scan_data.get('references', [])  # Fetch references for superscripts
    ...
    # Overall Summary section with superscripts for references
    doc.add_heading('Overall Summary:', level=2)
    if overall_summary.strip():
        overall_summary_with_references = overall_summary
        for i, reference in enumerate(references, 1):
            overall_summary_with_references = overall_summary_with_references.replace(f"[{i}]", f"{i}")  # Replace with superscript-like index
        doc.add_paragraph(overall_summary_with_references)
    else:
